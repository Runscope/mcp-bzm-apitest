...
class Environment(BaseModel):
    ...
    parent_environment_id: Optional[str] = Field(
        default=None,
        description="The parent environment id if this environment is inherited from another "
        "environment",
    )
    ...
    webhooks: Optional[List] = Field(
        default=None,
        description="List of webhooks URLs to be called for notifications after test run " "completion",
    )
    integrations: Optional[list[Dict]] = Field(
        default=None,
        description="List of 3rd party integrations like Slack, MS Teams configured for this"
        " environment",
    )
    emails: Optional[EmailSettings] = Field(
        default=None, description="Email notifications enabled for this environment"
    )
    preserve_cookies: bool = Field(description="Whether to preserve cookies between test steps")
    stop_on_failure: bool = Field(
        description="Whether to stop the test on the first failure and not execute the remaining steps"
    )
    verify_ssl: bool = Field(description="Whether to verify SSL certificates for HTTPS requests")
    http_version_support: str = Field(description="HTTP version support")
    force_h2c: bool = Field(description="Whether to force HTTP/2 cleartext (h2c) connections")
    regions: list[str] = Field(description="List of cloud regions from which the test will be executed")
    ...
    is_client_certificate_used: Optional[bool] = Field(
        default=False,
        description="Indicates whether client certificate authentication is used in this" " environment",
    )
    ...
    auth_type: Optional[str] = Field(
        default=None,
        description="Type of authentication used in this environment,"

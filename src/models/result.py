...
class SubtestRequestResult(BaseModel):
    ...
    assertions: Optional[Dict[str, Any]] = Field(default=None, description="Assertions statistics and details")
...
class SubtestDetail(BaseModel):
    ...
    test_id: str = Field(alias="test_uuid", description="Id of the parent test which is actually executed by the subtest step")
...
class RequestResult(BaseModel):
    ...
    variables: Optional[List[Dict[str, Any]]] = Field(default=None, description="Variables extracted in this request")
    assertions: Optional[List[AssertionResult]] = Field(default=None, description="Assertions for this request")
    scripts: Optional[List[ScriptResult]] = Field(default=None, description="Scripts executed in this request")
    ...
    timings: Optional[Dict[str, Any]] = Field(default=None, description="Timing information for the request")
...
class TestResult(BaseModel):
    ...
    variables_passed: int = Field(description="Number of variables that were extracted successfully during the test run")
    variables_failed: int = Field(description="Number of variables that failed to be extracted during the test run")
    ...
    finished_at: Optional[float] = Field(default=None, description="The timestamp when the test run finished (Unix timestamp)")
    ...
    source: str = Field(description="The source of the test run. Possible values are - 'manual', 'scheduled', 'trigger_url'")
    region: Optional[str] = Field(default=None, description="The region where the test run was executed")
    run_by: Optional[str] = Field(default=None, description="The name of the user who initiated the test run")
    environment_id: str = Field(description="The environment ID used for this test run")
    environment_name: Optional[str] = Field(default=None, description="The environment name used for this test run")
    requests: Optional[List[RequestResult]] = Field(default=None, description="List of individual request results within this test run")

    # Additional fields from the response
    agent: Optional[str] = Field(default=None, description="If test is executed on the private location. Only valid if region is null")
    parent_test_uuid: Optional[str] = Field(default=None, description="Parent test UUID in case of subtest execution only")
    ...
    agent_expired: Optional[bool] = Field(default=None, description="Whether agent(private location) expired")
    subtest_detail: Optional[List[SubtestDetail]] = Field(default=None, description="Detailed subtest results")

class FailureDetails(BaseModel):
    """Failure details for bucket-level test run."""
    failed_test_runs: int = Field(alias="tests_failed", description="Number of failed test runs in the bucket-level test run")
    assertions_failed: int = Field(description="Total Number of assertions that failed across all the test runs")
    scripts_failed: int = Field(description="Total Number of scripts that failed across all the test runs")
    requests_failed: int = Field(description="Total Number of requests that failed across all the test runs")


class BucketLevelTestResult(BaseModel):
    """Bucket-level test run result."""
    bucket_level_test_run_id: str = Field(alias="uuid", description="Unique identifier for the bucket-level test run")
    ...
    total_requests: Optional[int] = Field(default=None, description="Total number of requests across all test runs")
    total_scripts: Optional[int] = Field(default=None, description="Total number of scripts across all test runs")
    total_assertions: Optional[int] = Field(default=None, description="Total number of assertions across all test runs")
    finished_at: Optional[float] = Field(default=None, description="Timestamp when the bucket-level test run finished")
    total_duration_in_sec: Optional[float] = Field(default=None, description="Total execution time of the bucket-level test run in seconds")
...
class TestExecutionRuns(BaseModel):
    ...
    region: Optional[str] = Field(default=None, description="The region i.e. public location where the test was executed")
    agent: Optional[str] = Field(default=None, description="The agent i.e. private Location where test is running")
    environment_id: str = Field(description="The environment ID used for this test run")
    test_run_url: str = Field(alias="api_test_run_url", description="The URL of the api to fetch the results of this test run")
...
class TestExecution(BaseModel):
    ...
    bucket_level_test_run_id: Optional[str] = Field(default=None, alias="runs_id",
        description="The unique identifier for the bucket-level test run")
    bucket_level_test_run_url: Optional[str] = Field(alias="consolidated_test_results_url",
    ...
    total_runs_failed: int = Field(alias="runs_failed", description="Total number of test runs failed to start due to any issue/error")
    total_runs: int = Field(alias="runs_total", description="Total number of test runs. An individual test run can also be executed multiple times if it is supposed to run on the multiple locations configured in the environment.")
    test_results: List[TestExecutionRuns] = Field(alias="runs", description="List of details about the individual test runs started as part of this test run")
